...
export default {
  ...
  created() {
    this.initialPassword = this.$route.query.password
  ...
  methods: {
    ...
    onVerificationCodeInput(event) {
      ...
      if (this.verificationCode.length === 6) {
        if (this.initialPassword === this.verificationCode) {
          this.$router.push({
          path:'set-results',
          query:{}
        })
      ...
      if (this.verificationCode.length === 6 && this.initialPassword !== this.verificationCode) {
        console.log('进来了')
         tip('两次输入密码不一致，请重新输入')
      }
    },
